...
// The same four changes the extension-host harness makes (.vscode-test.mjs says why these
...
// `b → b2`, next to its added `c`; it adds a small binary file, `logo.png` — a `\0` in
// the text is the NUL byte that makes git call a file binary — which a click opens as a
// file rather than a diff; it adds a text file with a space, `#`, `ü` and `?` in its
// name, whose diff must open like any other (E11); and it deletes trunk's `f`, so there
// is a `D  f` row whose diff is content on the left and nothing on the right (E9).
// The `?` is not allowed in a Windows file name, so this script — and the test harness,
// which writes the same file — runs on macOS and Linux only, the two platforms CI covers.
